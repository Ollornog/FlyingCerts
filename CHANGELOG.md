...
- The test CA ran from `latest`, so two runs could get two different images.
  It is now pinned by digest in both workflows and in the package
  documentation, and the readiness check waits for `renewalInfo` in the
  directory rather than merely for an answer.
